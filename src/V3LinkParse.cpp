...
class LinkParseVisitor final : public VNVisitor {
    ...
    void findPatternVarsRecurse(AstNode* nodep, std::vector<AstPatternVar*>& patVars) {
        if (!nodep) return;
        if (AstPatternVar* const patVarp = VN_CAST(nodep, PatternVar)) {
            ...
            for (AstNode* childp = patp->itemsp(); childp; childp = childp->nextp()) {
                findPatternVarsRecurse(childp, patVars);
            }
        } else if (AstPatMember* const membrp = VN_CAST(nodep, PatMember)) {
            findPatternVarsRecurse(membrp->lhssp(), patVars);
        }
    }
    ...
    void findPatternVarsInMatchesRecurse(AstMatches* matchesp, std::vector<AstPatternVar*>& patVars) {
        if (!matchesp) return;
        // Find pattern variables in this matches' pattern
        findPatternVarsRecurse(matchesp->patternp(), patVars);
    ...
    void createPatternVariables(AstCase* casep) {
        for (AstCaseItem* itemp = casep->itemsp(); itemp;
            ...
            for (AstNode* condp = itemp->condsp(); condp; condp = condp->nextp()) {
                findPatternVarsRecurse(condp, patVars);
            }
            // Create local variables for all pattern variables found
            if (patVars.empty()) continue;
            AstNode* stmtsp = nullptr;
            for (AstPatternVar* const patVarp : patVars) {
                ...
                AstVar* const varp
                    = new AstVar{fl, VVarType::BLOCKTEMP, patVarp->name(), VFlagBitPacked{}, 1};
                varp->funcLocal(true);
                stmtsp = AstNode::addNext<AstNode, AstNode>(stmtsp, varp);
            }
            // Wrap in AstBegin block to create a new scope for the pattern variables
            if (itemp->stmtsp()) {
                AstNode* const origStmtsp = itemp->stmtsp()->unlinkFrBackWithNext();
                stmtsp->addNext(origStmtsp);
            }
    ...
    // Process if-matches: create local variables for pattern variables wrapped in a scope block
    void processIfMatches(AstIf* nodep) {
        AstMatches* const matchesp = VN_AS(nodep->condp(), Matches);
        ...
        std::vector<AstPatternVar*> patVars;
        findPatternVarsInMatchesRecurse(matchesp, patVars);
        ...
        const bool hasGuard
            = matchesp->guardp() || VN_IS(matchesp->lhsp(), Matches);
        ...
        for (AstPatternVar* const patVarp : patVars) {
            ...
            AstVar* const varp
                = new AstVar{fl, VVarType::BLOCKTEMP, patVarp->name(), VFlagBitPacked{}, 1};
            varp->funcLocal(true);
            varDeclsp = AstNode::addNext<AstNode, AstNode>(varDeclsp, varp);
        }
